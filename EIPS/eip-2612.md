...
Many in the wild implementations of this standard have chosen to memoize the calculation of the domain separator by setting it in the constructor. While this provides non trivial gas savings, it does mean that the domain separator will not be updated in case of a chain split, allowing an attacker to replay messages intended for one fork to another. This could be problematic in case the target of the permit has different behaviours on each fork (e.g. if the trusted address is an owned proxy). Implementors should weigh the risk of the attack against the cost of mitigating the issue. Potential mitigation strategies include:

- calculating the domain separator each time
- enforcing allowed chain ID's within `permit` (e.g. `require(chainID = 1)`)
- automatically recomputing the domain separator only when the chainID changes (see [here](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/53516bc555a454862470e7860a9b5254db4d00f5/contracts/token/ERC20/ERC20Permit.sol) for an example)
